...
# Added functionality: Commands and goals are listened and syncronized in database.
...
def handle_commands():
    global robotCommands_ref, robotCommands_entries, goals_ref
    
    for docx in robotCommands_entries:
...
def check_for_goal():
    global goals_ref, is_initialized
...
def monitor_collection():
    global robotCommands_col, goals_col
    robotCommands_col = db.collection("robotCommands")
    goals_col = db.collection("goals")
...
# start processing goals  ->  goals collection'ınını dinlemeye basla   startGoals
# stop processing goals    -> simdiki goal'ı sil(2) goali sildiginde currentlyProcessedOrder'i de ara, goal id'de bir sey varsa sil, goals collection'ınını dinlemeyi bırak(1), baslangic yerine don(3)    stopGoals
