...
is_initial_stopped = False

# Added functionality: Buggy behaviour of Cancel Current Goal button is fixed and completed.

    
def handle_commands():
    global robotCommands_ref, robotCommands_col, goals_ref, goals_col, nav, col_query2, is_initial_stopped

    for docx in robotCommands_ref:
        # Retrieve the first document
        current_doc_id = docx.id
        data = robotCommands_col.document(current_doc_id).get().to_dict()
        command = data.get("command")
        robotCommands_col.document(current_doc_id).delete()
        ...
        if command == "Start Automation":
            ...
            print("Process the command: Stop Automation")
            # Cancel the goal
            nav.cancelTask()
        ...
        elif command == "Stop Current Goal":
            print("Process the command: Stop Current Goal")
            # Cancel the goal
            nav.cancelTask()
            col_query2.unsubscribe()
            handle_stop_currGoal()
            
        elif command == "Resume Current Goal":
            print("Process the command: Resume Current Goal")
            if is_initial_stopped == True:
                is_initial_stopped = False
                go_to_initial_location()
            
            else: 
                handle_resume_currGoal()
                is_initial_stopped = False
                

            col_query2 = goals_col.on_snapshot(on_snapshot_goals)
            
        elif command == "Cancel Current Goal":
            print("Process the command: Cancel Current Goal")
            is_initial_stopped = False
            nav.cancelTask()
            
            handle_cancel_currGoal()
            go_to_initial_location()
            col_query2 = goals_col.on_snapshot(on_snapshot_goals)
...
def handle_cancel_currGoal():  
    ...
    for docx in goals_ref:
        ...
        if query:
            for doc in query:
                new_id = doc.id
                currentlyProcessedOrder_col.document(new_id).update({"orderStatus": "Cancelled"})
...
def handle_resume_currGoal():  # arrived yap ve current goal sil, + 
    global currentlyProcessedOrder_ref, currentlyProcessedOrder_col, currentlyProcessedOrder_entries, goals_ref, goals_col, is_initial_stopped
    
    # Get only the first order from the orders list, that's why break is put immediately
    for docx in goals_ref:
        # Retrieve the first document
        current_doc_id = docx.id
        data = goals_col.document(current_doc_id).get().to_dict()
        ...
        if query:
            for doc in query:
                new_id = doc.id
                currentlyProcessedOrder_col.document(new_id).update({"orderStatus": "OnItsWayx"})
...
def handle_stop_currGoal():  # arrived yap ve current goal sil, + 
    global currentlyProcessedOrder_ref, currentlyProcessedOrder_col, currentlyProcessedOrder_entries, goals_ref, goals_col, is_initial_stopped
    ...
    for docx in goals_ref:
        ...
        if query:
            for doc in query:
                new_id = doc.id
                currentlyProcessedOrder_col.document(new_id).update({"orderStatus": "Stopped"})
                print("Updated in currentlyProcessedOrder!")
                break
...
# cancel current operation -> cancel current task(2) ve simdiki goal'ı sil(3), table0'a git(4), Goal cancel oldugunda ilgili orderID'nin durumunu "Cancelled" yap. Ve o entryi sil currprocessingOrders'dan(5) cancelCurrentGoal
